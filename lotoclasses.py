...
class Person:
    ...
    def delete(self,n,actions):
        ...
        '''
        # card = self.my_card
        if actions == 'y' and any(n in sl for sl in self.my_card):
            res = 0
            for i in range(3):
                if n in self.my_card[i]:
                    self.my_card[i][self.my_card[i].index(n)] = '-'
                    self.count += 1




        elif actions == 'n' and not any(n in sl for sl in self.my_card):
            res = 0
        else:
            res = 1
        return res
...
class Game:
    ...
    def game_start(self):
        ...
        while True:
            ...
            if (len(set(self.players[i].my_card[0] + self.players[i].my_card[1] + self.players[i].my_card[2]))) < 3:
